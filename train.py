...
from xlm.data.image_fusion import ImageFusion
...
def get_parser():
    ...
    parser.add_argument("--lambda_bt", type=str, default="1",
                        help="BT coefficient")
    parser.add_argument("--lambda_ra", type=str, default="1",
                        help="RA coefficient")
    # parser.add_argument("--lambda_tcma_bt", type=str, default="1",
    #                     help="tcma BT coefficient")
    # parser.add_argument("--lambda_tcma_ae", type=str, default="1",
    #                     help="tcma AE coefficient")
    ...
    parser.add_argument("--ra_steps", type=str, default="",
                        help="relational token image steps")
    # parser.add_argument("--tcma_bt_steps", type=str, default="",
    #                     help="tcma_bt_step")
    # parser.add_argument("--tcma_ae_steps", type=str, default="",
    #                     help="tcma_ae_step")
    ...
    parser.add_argument("--image_dim", type=int, default=768,
                        help="image embs dim")
    parser.add_argument("--image_id", type=int, default=3,
                        help="image id in langs[]")
    parser.add_argument("--image_embed_dropout_prob", type=float, default=0.1,
                        help="image linear dropout")
    parser.add_argument("--add_image", type=bool, default=False,
                        help="add image or not")
    parser.add_argument("--first_time_label", type=bool_flag, default=True,
                        help="first batch in bt or not")
    parser.add_argument("--eval_add_image", type=bool_flag, default=False,
                        help="add image or not when evaluation")
    parser.add_argument("--max_align_image", type=int, default=3,
                        help="image embs dim")
    

    # image_ra
    parser.add_argument("--ra_image_dim", type=int, default=768,
                        help="image embs dim")
    parser.add_argument("--ra_image_embed_dropout_prob", type=float, default=0.1,
                        help="image linear dropout")
    parser.add_argument("--image_ra", type=bool, default=False,
                        help="add image ra or not")
    parser.add_argument("--relation_temperature", type=float, default=1,
                        help="relation temperature")

    
    parser.add_argument("--codes_path", type=str, default="",
                        help="bpe codes path")
    parser.add_argument("--vocab_path", type=str, default="",
                        help="bpe vocab path")
    
    parser.add_argument("--best_image", type=bool, default=False,
                        help="add best image ra or not")
    
    parser.add_argument("--best_top2_image", type=bool, default=False,
                        help="add best top2 image ra or not")
    
    parser.add_argument("--fake_image", type=bool, default=False,
                        help="add fake image or not")
    
    parser.add_argument("--sort_image_ratio", type=float, default=1.0,
                        help="radio * image num")
#     parser.add_argument("word_level_image", type=bool, default=False,
#                         help="add word level image or token level")
    parser.add_argument("--image_add_noise_radio", type=float, default=0.0,
                        help="radio * image num to add noise")

    parser.add_argument("--random_choice_image", type=bool, default=False,
                        help="random pick up images")
...
def main(params):
    ...
    if params.add_image or params.image_ra:
        image_fusion_lang1 = ImageFusion(params.langs[0], params)
        image_fusion_lang2 = ImageFusion(params.langs[1], params)
    # evaluation
    if params.eval_only:
#         scores = evaluator.run_all_evals(trainer)
        if params.add_image:
            scores = evaluator.run_all_evals(trainer, image_fusion_lang1, image_fusion_lang2)
        else:
            scores = evaluator.run_all_evals(trainer)
    ...
    for _ in range(params.max_epoch):
        ...
        while trainer.n_sentences < trainer.epoch_size:
            ...
            for lang1, lang2 in shuf_order(params.mlm_steps, params):
                if not params.add_image:
                    trainer.mlm_step(lang1, lang2, params.lambda_mlm)
                else:
                    trainer.tcma_mlm_step(lang1, lang2, params.lambda_mlm, image_fusion_lang1, image_fusion_lang2)
            ...
            for lang in shuf_order(params.ae_steps):
                if not params.add_image:
                    ...
                    trainer.tcma_mt_step(lang, lang, params.lambda_ae, image_fusion_lang1, image_fusion_lang2)
            # # image fusion denoising auto-encoder steps
            # for lang in shuf_order(params.tcma_ae_steps):
            #     trainer.tcma_mt_step(lang, lang, params.lambda_tcma_ae, image_fusion_lang1, image_fusion_lang2)
#             break
            ...
            for lang1, lang2, lang3 in shuf_order(params.bt_steps):
                if not params.add_image:
                    ...
                    trainer.tcma_bt_step(lang1, lang2, lang3, params.lambda_bt, image_fusion_lang1, image_fusion_lang2)
            
            # image relational alignment
            for lang in shuf_order(params.ra_steps):
                trainer.relational_token_image_step(lang, params.lambda_ra, image_fusion_lang1, image_fusion_lang2)
            
            
            params.first_time_label = False
#             print(stop)
            #
            # # back-translation steps
            # for lang1, lang2, lang3 in shuf_order(params.tcma_bt_steps):
            #     trainer.tcma_bt_step(lang1, lang2, lang3, params.lambda_tcma_bt, image_fusion_lang1, image_fusion_lang2)
            trainer.iter()
