...
#         self.tokenizer = pkuseg.pkuseg(model_name="default", postag=False, user_dict=NEVER_SPLIT_TAG)
        ...
        self.image_embs_keys = list(self.image_embs.keys())
#         self.image_bpe = True # 爬取图片时是否是token级别的爬取 token/word
    ...
    def load_image_embs(self):
        ...
        if self.params.best_image:
            ...
            with open("../../TCMA/clip/CLIP/new_"+self.lang1+"_image_embs_dict_cpu.pickle", "rb") as f_en:
                en_embs_dict = pickle.load(f_en)
            f_en_id = open("../../TCMA/new_"+self.lang1+"id2token_image.txt", "r", encoding="utf-8")
            en_id2token = {}
            while True:
            ...
            f_en_not_rgb_list = open("../../TCMA/clip/CLIP/new_"+self.lang1+"_not_rgb_list_cpu", "r", encoding="utf-8")
            en_not_rgb_list = set()
            while True:
            ...
            for en_key in en_embs_dict.keys():
                if en_key in en_not_rgb_list:
                    continue
                # id = en_id2token[en_key.split("/")[0].split("_")[a1]]
            ...
            if self.params.fake_image:
                with open("new_"+self.lang1+"_image_embs_fake.pickle", "rb") as f_fake:
                    fake_embs_dict = pickle.load(f_fake)
                for fake_key in fake_embs_dict.keys():
                    if fake_key in en_embs_dict.keys():
                        continue
                    else:
                        final_en_image_embs[fake_key] = [fake_embs_dict[fake_key].float()]
        sort_final_en_image_embs = {}
        image_embs_dict_len = len(final_en_image_embs)
        i_count = 0 
        max_count = image_embs_dict_len * float(self.params.sort_image_ratio)
        for token_key in final_en_image_embs.keys():
            i_count += 1
            if i_count > max_count:
                break
            sort_final_en_image_embs[token_key] = final_en_image_embs[token_key]

        noise_final_en_image_embs = {}
        if self.lang1 != "en":
            # if self.params.iamge_add_noise_radio == 1.0:
            j_count = 0
            j_max_count = image_embs_dict_len * float(self.params.image_add_noise_radio)
            for j_token_key in sort_final_en_image_embs.keys():
                if j_count >= j_max_count:
                    noise_final_en_image_embs[j_token_key] = sort_final_en_image_embs[j_token_key]
                else:
                    length = len(sort_final_en_image_embs[j_token_key])
                    noise_image = [torch.randn(768) for _ in range(length)]
                    noise_final_en_image_embs[j_token_key] = noise_image
                j_count += 1
        else:
            noise_final_en_image_embs = sort_final_en_image_embs
           
        print(len(noise_final_en_image_embs))
        return noise_final_en_image_embs
        ...
        for split_sent in split_sent_batch:
            ...
            for token in split_sent:
                if token in self.special_tags:
                    ...
                    if self.params.random_choice_image:
                        random_img_list = list(self.image_embs.get(random.choice(self.image_embs_keys), []))
                        words = [["<unk>"] for _ in range(
                            min(len(random_img_list), int(self.params.max_align_image)))]
                        all_image_in_one_sent.extend(random.sample(random_img_list,
                                                                   min(len(random_img_list),
                                                                       int(self.params.max_align_image))))

                    else:
                        words = [["<unk>"] for _ in range(
                            min(len(list(self.image_embs.get(token, []))), int(self.params.max_align_image)))]
                ...
                #     token_pos_idx = [pos_idx + a1]
                #     token_abs_idx = [abs_idx + a1]
                # else:
...
#             print("梯度", all_image_in_one_batch[0].requires_grad)
        ...
        for i in range(len(index_list)):
            ...
            if len(know_sent) < max_length:
                ...
                # know_sent[-a1] = self.eos_index
                seg = seg[:max_length]
                # seg[-a1] = self.lang1_id
                pos = pos[:max_length]
                # pos[-a1] = max_length - a1
...
#         print(all_image_in_one_batch[0].shape)
    ...
    def add_image_embs(self, sent_batch, dico):
        """
...
#         print("sent_batch", sent_batch)
        ...
        for sent in sent_batch:
            new_sent = ['</s>']
            new_sent.extend(sent[4:-4].split(" "))
            new_sent.append('</s>')
            split_sent_batch.append(new_sent)
#         print("split_sent_batch", split_sent_batch)
        # token_level的需要bpe
#         all_bpe_sent_batch = []
