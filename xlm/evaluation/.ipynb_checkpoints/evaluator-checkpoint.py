...
class Evaluator(object):
    ...
    def mask_out_with_image(self, x, lengths, rng, image_fusion):
        """
        Decide of random words to mask out.
        We specify the random generator to ensure that the test is the same at each epoch.
        """
        params = self.params
        dico = self.dico
        x, lengths, position_batch, visible_matrix_batch, language_batch, all_image_in_one_batch = image_fusion.add_image_embs_bpe(
                    x, lengths, dico=dico)
        slen, bs = x.size()
    ...
    def run_all_evals(self, trainer, image_fusion_lang1=None, image_fusion_lang2=None):
        """
        Run all evaluations.
        """
        params = self.params
        ...
        with torch.no_grad():
            ...
            for data_set in ['valid', 'test']:
                ...
                for lang1, lang2 in params.mlm_steps:
                    if image_fusion_lang1 is None:
                        self.evaluate_mlm(scores, data_set, lang1, lang2)
                    else:
                        self.evaluate_mlm_with_image(scores, data_set, lang1, lang2, image_fusion_lang1, image_fusion_lang2)
                ...
                for lang1, lang2 in set(params.mt_steps + [(l2, l3) for _, l2, l3 in params.bt_steps]):
                    ...
                    self.evaluate_mt(scores, data_set, lang1, lang2, eval_bleu, image_fusion_lang1, image_fusion_lang2)
    ...
    def evaluate_mlm_with_image(self, scores, data_set, lang1, lang2, image_fusion_lang1, image_fusion_lang2):
        """
        Evaluate perplexity and next word prediction accuracy.
        """
        params = self.params
...
#             x, y, pred_mask = self.mask_out(x, lengths, rng)
            # new pred
            x, y, pred_mask,lengths, positions, visible_matrix_batch, langs, all_image_in_one_batch = self.mask_out_with_image(x, lengths, rng, image_fusion)
...
#             model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False)
            word_scores, loss = model('predict', tensor=tensor, pred_mask=pred_mask, y=y, get_scores=True)
...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu, image_fusion_lang1=None, image_fusion_lang2=None):
        """
        Evaluate perplexity and next word prediction accuracy.
        """
        params = self.params
...
#                     original_sents, dico=dico)
                x1, len1, position_batch, visible_matrix_batch, language_batch, all_image_in_one_batch = image_fusion.add_image_embs_bpe(
                    x1, len1, dico=dico)
                ...
                # cuda
                x1, len1, langs1, x2, len2, langs2, y, position_batch, visible_matrix_batch = to_cuda(x1, len1, langs1,
                                                                                                      x2, len2, langs2,
                                                                                                      y, position_batch,
                ...
                enc1 = encoder('image_fusion_fwd', x=x1, lengths=len1, positions=position_batch, langs=langs1,
                                    causal=False, vm=visible_matrix_batch, image_embs=all_image_in_one_batch)
            ...
            if eval_bleu:
                max_len = int(1.3 * len1.max().item() + 10)
#                 print(max_len)
