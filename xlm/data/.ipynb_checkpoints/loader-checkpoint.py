...
def load_mono_data(params, data):
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            if lang in params.ae_steps or lang in params.bt_src_langs or lang in params.mlm_lgs:
                print("batched dataset loading")
...
def check_data_params(params):
    ...
    params.mlm_lgs = [element for sublist in mlm_steps for element in sublist]
    ...
    # denoising auto-encoder steps
    params.ra_steps = [s for s in params.ra_steps.split(',') if len(s) > 0]

    # tcma denoising auto - encoder steps
#     params.tcma_ae_steps = [s for s in params.tcma_ae_steps.split(',') if len(s) > 0]
#     assert all([lang in params.langs for lang in params.tcma_ae_steps])
#     assert len(params.tcma_ae_steps) == len(set(params.tcma_ae_steps))
#     assert len(params.tcma_ae_steps) == 0 or not params.encoder_only
    ...
    # tcma back-translation steps
#     params.bt_steps = [tuple(s.split('-')) for s in params.tcma_bt_steps.split(',') if len(s) > 0]
#     assert all([len(x) == 3 for x in params.tcma_bt_steps])
#     assert all([l1 in params.langs and l2 in params.langs and l3 in params.langs for l1, l2, l3 in params.tcma_bt_steps])
#     assert all([l1 == l3 and l1 != l2 for l1, l2, l3 in params.tcma_bt_steps])
#     assert len(params.tcma_bt_steps) == len(set(params.tcma_bt_steps))
#     assert len(params.tcma_bt_steps) == 0 or not params.encoder_only
#     params.bt_src_langs = [l1 for l1, _, _ in params.tcma_bt_steps]
