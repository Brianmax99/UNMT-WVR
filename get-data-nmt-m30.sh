...
if [ "$SRC" == "de" -a "$TGT" == "en" ]; then
  PARA_SRC_VALID=$PARA_PATH/dev/test_2016_flickr.de
  PARA_TGT_VALID=$PARA_PATH/dev/test_2016_flickr.en
  PARA_SRC_TEST=$PARA_PATH/dev/test_2017_flickr.de
  PARA_TGT_TEST=$PARA_PATH/dev/test_2017_flickr.en
...
echo "Tokenizing valid and test data..."
eval "cat $PARA_SRC_VALID.orig | $SRC_PREPROCESSING > $PARA_SRC_VALID"
eval "cat $PARA_TGT_VALID.orig | $TGT_PREPROCESSING > $PARA_TGT_VALID"
eval "cat $PARA_SRC_TEST.orig  | $SRC_PREPROCESSING > $PARA_SRC_TEST"
eval "cat $PARA_TGT_TEST.orig  | $TGT_PREPROCESSING > $PARA_TGT_TEST"
