...
class Trainer(object):
    ...
    def __init__(self, data, params):
        ...
        self.stats = OrderedDict(
            ...
            [('TCMA_MLM-%s' % l, []) for l in params.langs] +
            ...
            [('TCMA_AE-%s' % lang, []) for lang in params.ae_steps] + 
            [('TCMA_BT-%s-%s-%s' % (l1, l2, l3), []) for l1, l2, l3 in params.bt_steps] +
            [('RA-%s' % lang, []) for lang in params.ra_steps]
    ...
    def mask_out_with_image(self, x, lengths, image_fusion):
        """
        Decide of random words to mask out, and what target they get assigned.
        """
        params = self.params
        
        x, lengths, position_batch, visible_matrix_batch, language_batch, all_image_in_one_batch = image_fusion.add_image_embs_bpe(x, lengths, self.data["dico"])
        slen, bs = x.size()
...
#         print(pred_mask)
        ...
        # do not predict any image
        pred_mask[language_batch == params.image_id] = 0
        ...
        return x, _x_real, pred_mask,lengths, position_batch, visible_matrix_batch, language_batch, all_image_in_one_batch
    ...
    def generate_batch(self, lang1, lang2, name):
        ...
        if lang2 is None:
            if name == "pred_image":
                x, lengths = self.get_batch(name, lang1)
            else:
    ...
    def reload_checkpoint(self):
        ...
        for name in self.MODEL_NAMES:
            getattr(self, name).load_state_dict(data[name], strict=False)
...
#         print(pred_mask)
    ...
    def tcma_mlm_step(self, lang1, lang2, lambda_coeff, image_fusion_lang1, image_fusion_lang2):
        """
        Masked word prediction step.
        MLM objective is lang2 is None, TLM objective otherwise.
        """
        ...
        image_fusion = image_fusion_lang1 if lang1 == params.langs[0] else image_fusion_lang2

        # generate batch / select words to predict
        x, lengths, positions, langs, _ = self.generate_batch(lang1, lang2, 'pred_image',)
        x, lengths, positions, langs, _ = self.round_batch(x, lengths, positions, langs)
        x, y, pred_mask,lengths, positions, visible_matrix_batch, langs, all_image_in_one_batch = self.mask_out_with_image(x, lengths, image_fusion)
        pred_mask = pred_mask.bool()
...
#         print(y)
        ...
        # cuda
        x, y, pred_mask, lengths, positions, visible_matrix_batch, langs = to_cuda(x, y, pred_mask, lengths, positions, visible_matrix_batch, langs)
        all_image_in_one_batch = [to_cuda(i) for i in all_image_in_one_batch]

        # forward / loss
        tensor = model('image_fusion_fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False,  vm=visible_matrix_batch, image_embs=all_image_in_one_batch)
        _, loss = model('predict', tensor=tensor, pred_mask=pred_mask, y=y, get_scores=False)
        self.stats[('TCMA_MLM-%s' % lang1) if lang2 is None else ('TCMA_MLM-%s-%s' % (lang1, lang2))].append(loss.item())
        loss = lambda_coeff * loss
...
class EncDecTrainer(Trainer):
    ...
    def get_original_sentences(self, x, lengths):
        # 得到原来的句子
        sentences = convert_to_text(x, lengths, self.data["dico"], self.params)
        original_sentences = []
        for sentence in sentences:
            original_sentences.append("</s>" + re.sub("(@@ )|(@@ ?$)", "", sentence) + "</s>")
        return original_sentences


    def tcma_mt_step(self, lang1, lang2, lambda_coeff, image_fusion_lang1, image_fusion_lang2):
        """
        Machine translation step.
        Can also be used for denoising auto-encoding.
        """
        ...
        # 对x1加入图片
        original_sents = self.get_original_sentences(x1, len1)
#         print(original_sents)
#         print("original_len1", len1)
        image_fusion = image_fusion_lang1 if lang1 == params.langs[0] else image_fusion_lang2
        dico = self.data["dico"]
        x1, len1, position_batch, visible_matrix_batch, language_batch, all_image_in_one_batch = image_fusion.add_image_embs(
            original_sents, dico=dico)
#         x1, len1, position_batch, visible_matrix_batch, language_batch, all_image_in_one_batch = image_fusion.add_image_embs_bpe(x1, len1, dico)
#         print("new", len1)
#         print("image", len(all_image_in_one_batch))
        langs1 = language_batch
        langs2 = x2.clone().fill_(lang2_id)
        ...
        # cuda
        x1, len1, langs1, x2, len2, langs2, y, position_batch, visible_matrix_batch = to_cuda(x1, len1, langs1, x2, len2, langs2, y, position_batch, visible_matrix_batch)
        # for i in all_image_in_one_batch:
        #     i = to_cuda(i)
        all_image_in_one_batch = [to_cuda(i) for i in all_image_in_one_batch]
#         print(all_image_in_one_batch[0].shape)
        # encode source sentence
        enc1 = self.encoder('image_fusion_fwd', x=x1, lengths=len1, positions=position_batch, langs=langs1, causal=False, vm=visible_matrix_batch, image_embs=all_image_in_one_batch)
        enc1 = enc1.transpose(0, 1)
        ...
        # loss
        _, loss = self.decoder('predict', tensor=dec2, pred_mask=pred_mask, y=y, get_scores=False)
        self.stats[('TCMA_AE-%s' % lang1) if lang1 == lang2 else ('MT-%s-%s' % (lang1, lang2))].append(loss.item())
        loss = lambda_coeff * loss
    ...
    def tcma_bt_step(self, lang1, lang2, lang3, lambda_coeff, image_fusion_lang1, image_fusion_lang2):
        """
        Back-translation step for machine translation.
        """
        assert lambda_coeff >= 0
        ...
        image_fusion1 = image_fusion_lang1 if lang1 == params.langs[0] else image_fusion_lang2
        image_fusion2 = image_fusion_lang1 if lang2 == params.langs[0] else image_fusion_lang2
        dico = self.data["dico"]
        ...
        if not params.first_time_label:
            original_sents = self.get_original_sentences(x1, len1)
            align_x1, align_len1, align_position_batch, align_visible_matrix_batch, align_language_batch, align_all_image_in_one_batch = image_fusion1.add_image_embs(
                original_sents, dico=dico)
#             align_x1, align_len1, align_position_batch, align_visible_matrix_batch, align_language_batch, align_all_image_in_one_batch = image_fusion1.add_image_embs_bpe(
#                 x1, len1, dico=dico)
            align_x1, align_len1, align_position_batch, align_visible_matrix_batch, align_language_batch = to_cuda(align_x1, align_len1, align_position_batch, align_visible_matrix_batch, align_language_batch)
            align_all_image_in_one_batch = [to_cuda(i) for i in align_all_image_in_one_batch]
            # generate a translation
            with torch.no_grad():
                # evaluation mode
                self.encoder.eval()
                ...
                # encode source sentence and translate it
                enc1 = _encoder('image_fusion_fwd', x=align_x1, lengths=align_len1, langs=align_language_batch, causal=False,
                                positions=align_position_batch, vm=align_visible_matrix_batch, image_embs=align_all_image_in_one_batch)
                enc1 = enc1.transpose(0, 1)
                x2, len2 = _decoder.generate(enc1, align_len1, lang2_id, max_len=int(1.3 * len1.max().item() + 5))
                # langs2 = x2.clone().fill_(lang2_id)

                # 对x2加对齐
                original_sents = self.get_original_sentences(x2, len2)
                x2, len2, position_batch, visible_matrix_batch, language_batch, all_image_in_one_batch = image_fusion2.add_image_embs(
                    original_sents, dico=dico)
#                 x2, len2, position_batch, visible_matrix_batch, language_batch, all_image_in_one_batch = image_fusion2.add_image_embs_bpe(
#                     x2, len2, dico=dico)
                langs2 = language_batch
...
#                     x2, len2, dico=dico)
                ...
                # to_cuda
        x2, len2, langs2, position_batch, visible_matrix_batch = to_cuda(x2, len2, langs2, position_batch,
                                                                                 visible_matrix_batch)
        all_image_in_one_batch = [to_cuda(i) for i in all_image_in_one_batch]

        # encode generate sentence
        enc2 = self.encoder('image_fusion_fwd', x=x2, lengths=len2, langs=langs2, causal=False, positions=position_batch,
                                    vm=visible_matrix_batch, image_embs=all_image_in_one_batch)
        enc2 = enc2.transpose(0, 1)
        ...
        # loss
        _, loss = self.decoder('predict', tensor=dec3, pred_mask=pred_mask, y=y1, get_scores=False)
        self.stats[('TCMA_BT-%s-%s-%s' % (lang1, lang2, lang3))].append(loss.item())
        loss = lambda_coeff * loss
    ...
    def relational_token_image_step(self, lang1, lambda_coeff, image_fusion_lang1, image_fusion_lang2):
        assert lambda_coeff >= 0
        if lambda_coeff == 0:
            return
        params = self.params
...
#         lang2_id = params.lang2id[lang2]
        ...
        # generate source batch
        x1, len1 = self.get_batch('ra', lang1)
        langs1 = x1.clone().fill_(lang1_id)
        x1, len1, langs1 = to_cuda(x1, len1, langs1)
#         langs2 = x2.clone().fill_(lang2_id)
        
        align_all_image_in_one_batch, all_image_token_pos_in_one_batch  = image_fusion.add_image_relational_bpe(
                x1, len1, dico=dico)
        all_image_token_pos_in_one_batch = [to_cuda(i) for i in all_image_token_pos_in_one_batch]
        align_all_image_in_one_batch = [to_cuda(i) for i in align_all_image_in_one_batch]
        
        loss = self.encoder('image_relational_alignment', x=x1, lengths=len1, langs=langs1, causal=False,
                                image_embs=align_all_image_in_one_batch, image_pos=all_image_token_pos_in_one_batch)
        
        self.stats[('RA-%s' % (lang1))].append(loss.item())
        loss = lambda_coeff * loss
...
#         self.n_sentences += params.batch_size
#         self.stats['processed_s'] += len1.size(0)
#         self.stats['processed_w'] += (len1 - a1).sum().item()
